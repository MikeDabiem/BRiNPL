<form enctype="multipart/form-data" class="feedback__form position-relative" id="contact-form">
    ...
    if ($services->have_posts()) : while ($services->have_posts()) : $services->the_post(); ?>
        <div class="service__item d-inline-block">
            <input type="checkbox" name="form-service[]" id="form-<?= the_ID(); ?>" class="service__item-input d-none" value="<?= the_ID(); ?>">
            <label for="form-<?= the_ID(); ?>" class="service__item-label transition-default"><?php the_title(); ?></label>
        </div>
    <?php endwhile;
    else: endif;
    wp_reset_query(); ?>
